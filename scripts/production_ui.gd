...
func _ready():
	panel.hide()
	# On se connecte au manager pour savoir quand un bâtiment est sélectionné
	# Note: On suppose que le manager est dans la scène et accessible (via groupe ou nom)
...
func _on_batiment_change(bat):
	# On se déconnecte de l'ancien bâtiment si connecté
	...
	if current_batiment and current_batiment.get("equipe") == 0: # 0 = JOUEUR
		panel.show()
		_update_queue_display(0, 0)
		# Se connecter au nouveau bâtiment pour l'état de la file
...
func _on_btn_support_pressed():
	if current_batiment: current_batiment.demander_production(3)

func _on_btn_heal_pressed():
	if current_batiment: current_batiment.demander_production(4)

func _on_btn_anti_armor_pressed():
	if current_batiment: current_batiment.demander_production(5)

func _on_btn_mortar_pressed():
	if current_batiment: current_batiment.demander_production(6)
